...
def send_mail(message, all=0, fail=0, error=0):
    env = 1  # 0为发送给全体研发，1为发送给自己，2为发送测试组
    from_addr = "****************"
    password = "**********"
    if env == 0:
    ...
    msg = MIMEMultipart()
    msg['From'] = formataddr(["SDK自动化测试报告", "no-reply@autotest.udesk.cn"])
    msg['Subject'] = Header('SDK自动化测试报告', 'utf-8').encode()
    if env == 2:
    ...
    html = """\
    <html><body><h3 style="font-size:20px">----- SDK自动化测试完成 -----</h3>
    ...
    <p>报告详情请点击： <a href="http://autotest.udeskt1.com/%s">SDK自动化测试报告</a>...附件预览会有乱码，请下载查看</p> 
    </body></html>
...
if __name__ == '__main__':
    i = 0
    if i == 0:
        runner = HTMLTestReportCN.HTMLTestRunner(stream=fp, title="SDK自动化测试报告", environment="http://brazil.udesk.cn",
                                                 tester="乔雪源",
                                                 description="用例执行情况")
    else:
        runner = HTMLTestReportCN.HTMLTestRunner(stream=fp, title="SDK自动化测试报告", environment="http://linapp.udeskt1.com",
                                                 tester="乔雪源",
                                                 description="用例执行情况")
    # 执行用例
    ...
    print('执行用例总数：', all_case)
    print('执行失败用例数：', fail_case)
    print('执行错误用例数：', error_case)
